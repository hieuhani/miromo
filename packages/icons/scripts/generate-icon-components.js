...
async function generate() {
  const iconSvgPaths = await glob(
    path.resolve(rootDir, 'src/sources/**/*.svg')
  );
  const rootComponentsDir = path.resolve(rootDir, 'src/output');
...
`
  ...
  for (svgPath of iconSvgPaths) {
        ...
        .map((_, e) => ({
          ...
          child:
            e.children && e.children.length
              ? elementToTree(cheerio.default(e.children))
              : undefined,
        }))
        .get();
    };
    ...
    await writeComponent(
      `src/output/Icon${pascalName}.tsx`,
      `// THIS FILE IS AUTO GENERATED. DO NOT MODIFY!
...
`
  ...
  await exec('eslint --ext .ts,.tsx ./src --fix');
}
